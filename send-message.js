...
async function addListener(userId,friendId) {
    ...
    await firebase.firestore().collection('messages').where("sender", "==", friendId).where("sentTo", "==", userId).where("timestamp","==",Date.now()).onSnapshot(async function(querySnapshot) {
